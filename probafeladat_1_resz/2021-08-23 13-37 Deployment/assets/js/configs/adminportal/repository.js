...
adminPortalWidgetCatalogPagingDropBox:{state:            (db)=>{ return {items: [{name: '10', on:true},{name: '20'},{name: '50'},{name: '100'}]};},},
...
adminPortalSystemConfigHorizontalTable:{init:      {            url:  (db) => `/api/v1/ExecuteMDX?$expand=Cells($select=Ordinal,FormattedValue;$expand=Members($select=Name))`,            type: 'POST',            body:  (db) => `{"MDX":"SELECT TM1SubsetToSet([zSYS Analogic Framework Parameter],\\"FlaskParameters\\") on ROWS, {[zSYS Analogic Framework Parameter Measure].[ParameterValue]} on COLUMNS  FROM [zSYS Analogic Framework Parameter] WHERE ([zSYS Analogic Framework Application List].[${db.selectedApplication}])"}`            ,            parsingControl: {                        type: 'matrix',                        length: 1,                        query:[                        (r, x) => {
...
adminPortalSystemConfigJSHorizontalTable:{init:      {            url:  (db) => `/api/v1/ExecuteMDX?$expand=Cells($select=Ordinal,FormattedValue;$expand=Members($select=Name))`,            type: 'POST',            body:  (db) => `{"MDX":"SELECT TM1SubsetToSet([zSYS Analogic Framework Parameter],\\"JavaScriptParameters\\") on ROWS, {[zSYS Analogic Framework Parameter Measure].[ParameterValue]} on COLUMNS  FROM [zSYS Analogic Framework Parameter] WHERE ([zSYS Analogic Framework Application List].[${db.selectedApplication}])"}`            ,            parsingControl: {                        type: 'matrix',                        length: 1,                        query:[                        (r, x) => {
                        return {value: r.Cells[x].Members[1].Name};
                    },(r, x) => {
                        return {value: r.Cells[x].FormattedValue, editable: true, ordinal: x};
                    }]	             }                              },cellEdit:      {            url:  (db) => `/api/v1/Cellsets('${db.cellsetId}')/Cells`,            type: 'PATCH',            body:  (db) => `{"Ordinal": ${db.adminPortalSystemConfigHorizontalTable.cellEdit.ordinal},"Value": \"${db.adminPortalSystemConfigHorizontalTable.cellEdit.value}\"}`                                                      },},
...
adminPortalWidgetConfigurationAddEventGridActionStatus:{state:            (db)=>{ return {items: [{name: '', on:true},{name: 'finished'},{name: 'started'}]};},},
adminPortalWidgetConfigurationAddEventGridButton:{launch:      {            url:  (db) => `/api/v1/Processes('zSYS Analogic UI Add New Event')/tm1.ExecuteWithReturn`,            type: 'POST',            body:  (db) => `{
            ...
            }`                        ,            validation: (db) => {if (db.adminPortalWidgetConfigurationAddEventGridAction.value === "" || db.adminPortalWidgetConfigurationAddEventGridEvent.value === "") {
...
adminPortalWidgetConfigurationAddListenGridAction:{state:            (db)=>{ return {items: [{name: 'refresh'},{name: 'refreshWithState'},{name: 'refreshWithWaitingForEvent'}]};},},
...
adminPortalWidgetConfigurationEditListenGridAction:{state:            (db)=>{ return {items: [{name: 'refresh', on: 'refresh' === v('adminPortalEventListenHorizontalTable.open.method')},{name: 'refreshWithState', on: 'refreshWithState' === v('adminPortalEventListenHorizontalTable.open.method')},{name: 'refreshWithWaitingForEvent', on: 'refreshWithWaitingForEvent' === v('adminPortalEventListenHorizontalTable.open.method')}]};},},
